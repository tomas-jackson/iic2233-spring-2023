...
class Corredor(Thread):
    ...
    def avanzar(self) -> None:
        # Completar
        self.posicion += (randint(Corredor.PORCENTAJE_MIN, Corredor.PORCENTAJE_MAX) / 100) * self.velocidad
    ...
    def intentar_capturar_tortuga(self) -> None:
        # Completar
        if self.lock_tortuga.acquire(blocking=False) is True:
        ...
        while self.__correr:
            with self.lock_verificar_tortuga:
                if self.senal_fin.is_set() is True:
                    return False
                if self.__posicion >= 100 and self.tiene_tortuga == True:
                    self.senal_fin.set()
                    self.lock_tortuga.release()
                    return True
